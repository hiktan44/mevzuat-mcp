...
class ExchangeRateTests(unittest.TestCase):
    def test_parse_bulletin_reads_rates_and_units(self):
        ...
        with self.assertRaises(xr.ExchangeRateError):
            bulletin.quote("XAU")


    def test_candidate_days_skip_weekends_and_start_the_day_before(self):
    ...
    def test_customs_quote_uses_previous_business_day_bulletin(self):
        friday = xr.archive_url(date(2026, 9, 4))
        service, seen = self._service({friday: FIXTURE})
    ...
    def test_customs_quote_walks_back_over_holidays_and_caches(self):
        fixture = FIXTURE.replace('Tarih="04.09.2026"', 'Tarih="27.08.2026"')
        thursday = xr.archive_url(date(2026, 8, 27))
        service, seen = self._service({thursday: fixture})
    ...
    def test_customs_quote_fails_clearly_when_no_bulletin(self):
        service, _ = self._service({})
        with self.assertRaises(xr.ExchangeRateError):
            asyncio.run(service.customs_quote("USD", date(2026, 9, 7)))


    def test_parse_registration_date_accepts_turkish_format(self):
