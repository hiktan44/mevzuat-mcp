...
`ZAI_API_KEY` tanımlıysa birincil sağlayıcı Z.ai'dir (görsel: `glm-5v-turbo`, `glm-4.6v`; metin: `glm-5.3`, `glm-5.3-flash`). Görsel modellerde "düşünme" adımı varsayılan olarak kapalıdır (`ZAI_VISION_THINKING=disabled`); bu, dakikalarca süren yanıtları önler. Her istek `LLM_REQUEST_TIMEOUT_SECONDS` (75 sn), birincil zincir `LLM_PRIMARY_BUDGET_SECONDS` (95 sn) ve tüm çağrı `LLM_TOTAL_DEADLINE_SECONDS` (150 sn) ile sınırlıdır; süre dolunca kullanıcı sağlayıcı adı içermeyen net bir hata görür. Z.ai yanıt vermezse kalan süre içinde sırasıyla doğrudan Google Gemini (`GEMINI_API_KEY`, modeller `GEMINI_MODELS`, varsayılan `gemini-3.8-flash` → `gemini-flash-latest`) ve OpenRouter zinciri (`OPENROUTER_API_KEY`, `OPENROUTER_FALLBACK_MODELS`) otomatik yedek olarak denenir; `LLM_FALLBACK_TO_GEMINI=0` / `LLM_FALLBACK_TO_OPENROUTER=0` bunları kapatır. `LLM_PRIMARY_PROVIDER=gemini` ile Gemini birincil sağlayıcı yapılabilir. Arayüzde sağlayıcı veya model adı gösterilmez; yalnızca "yapay zekâ analizi" ve güven düzeyi görünür.
