...
- `control_engine.extract_annex_scope`: aynı ekin bütün parçaları (`Ek-1/A`, `Ek-1/B`…) birleştiriliyor;
  ...
  **6 Eyl'de tamamlandı**: yasak ekler resmî tebliğ metinlerinden doğrulanıp `control_sources.json`'a
  gerekçeleriyle (`annex_note`) işlendi — 2026/3 Atıklar MADDE 4/(1) Ek-2/A ve Ek-2/B; 2026/23 Metal
  Hurdalar MADDE 2/(1) ve 4/(4) Ek-2; 2026/6 Kimyasallar MADDE 4/(1) Ek-1 ve MADDE 12/(1) Ek-3. 2026/6
  Ek-2 bilerek kontrol listesi bırakıldı: MADDE 9'daki yasak yalnız tek kullanımlık kaplar içindir.
...
- ✅ **GTİP bazlı ÖTV listeleri** (6 Eyl): 4760 sayılı Kanunun ekli (I)-(IV) sayılı listeleri resmî metinden 253 satır olarak ayrıştırıldı (`tax_lists.py`, `data/official/excise_tax_lists.json`), tarife sonucuna `excise_tax` alanı, `/api/tariff/excise`, MCP `lookup_excise_tax`, arayüzde ÖTV tablosu. Dipnot numaraları resmî PDF'te değerlere yapışık basıldığından (uygulanacak oran "10" + dipnot "59" → "1059") düz metin çıkarımı yanlış oran üretiyordu; span koordinatları ve punto filtresiyle üst simgeler ayıklandı. (III) sayılı listede hücreler birleşik basıldığı için oran gösterilmiyor, yalnız kapsam bildiriliyor. Kod listelerde yoksa fakat aynı pozisyonda satır varsa Armonize Sistem revizyonu uyarısı veriliyor (ör. akıllı telefon 8517.13; Kanun 8517.12 taşıyor).
